...
DEFAULT_MODEL_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "binary", "yolov8n-face.torchscript"
)
...
class Detector:
    ...
    def _preprocess(
        ...
        result = torch.zeros(
            (new_height + pad_height, new_width + pad_width, 3),
            device=self.device,
            dtype=torch.float32,
        )
        result[
        ...
        ] = F.interpolate(img, (new_width, new_height), mode="bilinear")
